...
namespace Linq
{
    ...
    public static class JoinOperations
    {
        ...
        public static IEnumerable<(string category, string productName)> JoinQuery()
        {
            ...
            var joinProducts = categories
            .Join(products,
                category => category,
                product => product.Category,
                (category, product) => (category, productName: product.ProductName));
        ...
        public static IEnumerable<(string category, IEnumerable<Product> productsName)> GroupJoinQuery()
        {
            ...
            .GroupJoin(products,
                category => category,
                product => product.Category,
                (category, matchingProducts) => (category, productsName: matchingProducts));
        ...
        public static IEnumerable<(string category, string productName)> LeftOuterJoin()
        {
            ...
            .GroupJoin(products,
                category => category,
                product => product.Category,
                (category, matchingProducts) =>
                    (category, productName: matchingProducts.Select(p => p.ProductName).DefaultIfEmpty("(No products)")))
            .SelectMany(result => result.productName.Select(productName => (result.category, productName)));
